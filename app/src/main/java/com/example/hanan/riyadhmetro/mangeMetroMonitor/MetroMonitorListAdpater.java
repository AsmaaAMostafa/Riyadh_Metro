...
public class MetroMonitorListAdpater extends RecyclerView.Adapter<MetroMonitorListAdpater.AssignedMetroViewHolder> {
    ...
    private void setData( Map<String, Object> metroMonitors,AssignedMetroViewHolder holder) {
        ...
        if (metroMonitors.size() != 0 && holder != null) {
            ...
            holder.mMonitor_name.setText(metroMonitors.get(NAME_FIELD).toString());
        }
    ...
    /**/
    private void deleteMetroMonitor(int position){
    ...
    private void deleteMetroMonitorSuccefully(){
        ...
        Toast.makeText(mContext,"The Metro Monitor has been deleted succefully!",Toast.LENGTH_SHORT).show();
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                if (constraint != null & mMetroMonitorsCopy != null & mMetroMonitorsCopy.size() > 0) {
                    ...
                    for (final Map<String, Object> metroMonitor : mMetroMonitorsCopy) {
                        if (metroMonitor.get(NAME_FIELD).toString().toLowerCase().contains(constraint.toString().toLowerCase())) {

                            searchResults.add(metroMonitor);
                            oReturn.values = searchResults;
                        } else if (metroMonitor.get(EMAIL_FIELD).toString().toLowerCase().contains(constraint.toString().toLowerCase())) {

                            searchResults.add(metroMonitor);
                            oReturn.values = searchResults;
                        }else if (metroMonitor.get(NATIONAL_ID_FIELD).toString().toLowerCase().contains(constraint.toString().toLowerCase())) {

                            searchResults.add(metroMonitor);
                            oReturn.values = searchResults;
                        }else if (metroMonitor.get(BIRTH_DATE_FIELD).toString().toLowerCase().contains(constraint.toString().toLowerCase())) {
