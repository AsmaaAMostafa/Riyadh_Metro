...
//        mMetroMonitorId  = intent.getStringExtra(ID_KEY_INTENT);
    ...
    /**/
    private void getMointorId(){

        String email = userInput.get(EMAIL_FIELD).toString();

        db.collection(COLLECTION_METRO_MONITOR).whereEqualTo("Email", email).get()
    ...
    private void setDateOnfields( Map<String, Object> metroMonitor){
        ...
        if(metroMonitor != null){
            editTextName.setText(metroMonitor.get(NAME_FIELD).toString());
            editTextEmail.setText(metroMonitor.get(EMAIL_FIELD).toString());
            editTextPassword.setText(metroMonitor.get(PASSWORD_FIELD).toString());

            editTextBirth.setText(metroMonitor.get(BIRTH_DATE_FIELD).toString());
            editTextNationalid.setText(metroMonitor.get(NATIONAL_ID_FIELD).toString());
    }
